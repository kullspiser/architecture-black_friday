...
echo ">>> 4. Добавление шардов в mongos router..."
...
echo ">>> 5. Включение шардирования для БД и коллекции..."
docker compose exec -T mongos_router mongosh --port 27020 --quiet <<EOF
...
echo ">>> 6. Заполнение данными (1000 документов)..."
docker compose exec -T mongos_router mongosh --port 27020 --quiet <<EOF
use somedb
for(var i = 0; i < 1000; i++) db.helloDoc.insertOne({age:i, name:"ly"+i})
EOF

echo ">>> 7. Проверка количества документов на шардах..."
docker compose exec -T shard1-1 mongosh --port 27018 --quiet <<EOF
use somedb
...
echo ">>> 8. Проверка статуса реплик..."
docker compose exec -T shard1-1 mongosh --port 27018 --quiet <<EOF
